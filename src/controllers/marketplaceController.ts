...
/**
 * Get all licensed IPs for marketplace (regardless of creator)
 * Returns only IPs that have licenses attached
 * 
 * Query parameters:
 * - commercialUse: Filter by commercial use flag (true/false)
 * - maxRoyalty: Maximum royalty percentage (0-100)
 ...
 * - limit: Items per page (default: 20, max: 100)
 */
export const getLicensedIPs = async (req: Request, res: Response) => {
  logger.info(`getLicensedIPs request with query: ${JSON.stringify(req.query)}`);
  ...
  try {
    ...
    // Build query filter - only show IPs with licenses attached
    const filter: any = {
      licenseTermsId: { $exists: true, $ne: null },
      licenseAttachedAt: { $exists: true, $ne: null }
    };
    ...
    const [ips, totalCount] = await Promise.all([
      MintToken.find(filter)
        .select('nonce ipId tokenId creatorAddress contentHash ipMetadataURI nftMetadataURI licenseTermsId licenseType royaltyPercent allowDerivatives commercialUse licenseTxHash licenseAttachedAt usedAt status')
        .sort({ licenseAttachedAt: -1 }) // Most recently licensed first
        .skip(skip)
        .limit(limitNum)
        .lean(),
      MintToken.countDocuments(filter)
    ...
    logger.info(`Successfully retrieved ${formattedIPs.length} licensed IPs (total: ${totalCount})`);
    ...
    logger.error(`Error fetching licensed IPs: ${error.message}`);
    return res.status(500).json({
      success: false,
      error: 'SERVER_ERROR',
      message: 'Failed to fetch licensed IPs'
    });
  }
};
